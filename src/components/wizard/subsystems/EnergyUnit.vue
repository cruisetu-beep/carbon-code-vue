  ...
  <div class="list-form-wrapper">
    <div v-for="(item, index) in items" :key="index" class="list-item-card">
      ...
      <div class="native-form">
        ...
        <div class="field">
          <label class="field-label">工业类型</label>
          <template v-if="industrialTypeOptions().length > 0">
            <select class="input" v-model="item.industrialType">
              <option v-for="opt in industrialTypeOptions()" :key="opt.key" :value="opt.key">{{ opt.value }}</option>
            </select>
          </template>
          <input v-else class="input" v-model="item.industrialType" placeholder="工业类型" />
        </div>
        <div class="field">
          <label class="field-label">工业代码</label>
          <template v-if="industrialCodeOptions().length > 0">
            <select class="input" v-model="item.industrialCode">
              <option v-for="opt in industrialCodeOptions()" :key="opt.key" :value="opt.key">{{ opt.value }}</option>
            </select>
          </template>
          <input v-else class="input" v-model="item.industrialCode" placeholder="工业代码" />
        </div>
        <div class="field">
...
const industrialTypeOptions = () => getColumnOptions(props.columns, ['industrialType'], [])
const industrialCodeOptions = () => getColumnOptions(props.columns, ['industrialCode'], [])

const convertItems = (rawItems) => {
  if (!Array.isArray(rawItems)) return []
  return convertItemsValuesToKeys(rawItems, {
    group: groupOptions(),
    industrialType: industrialTypeOptions(),
    industrialCode: industrialCodeOptions()
  })
}
